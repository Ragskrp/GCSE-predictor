...
import { DIAGNOSTIC_LENGTH, START_DIFFICULTY, calculateWeightedScore, confidenceBand, findBoundarySet, findWeakTopics, filterQuestions, gradeFromRawMark, nextDifficulty, projectRawMark, selectNextQuestion } from './engine/predictor';
import './styles.css';

const BOUNDARY_YEAR = 2026;
const BOUNDARY_SERIES = 'June';
...
function App() {
  ...
  const boundarySet = findBoundarySet(gradeBoundaries, { board, subject, tier }, BOUNDARY_YEAR, BOUNDARY_SERIES);
  const weightedScore = calculateWeightedScore(askedQuestions, answers);
  const projectedMark = projectRawMark(weightedScore, boundarySet);
  const grade = gradeFromRawMark(projectedMark, boundarySet);
  const confidence = confidenceBand(weightedScore, answers.length);
  const weakTopics = useMemo(() => findWeakTopics(askedQuestions, answers), [askedQuestions, answers]);

  const start = () => {
    if (available.length < DIAGNOSTIC_LENGTH || !boundarySet) return;
  ...
  const answer = () => {
    ...
    const next = selectNextQuestion(available, nextUsed, newDifficulty);
    if (!next) {
      setScreen('result');
      return;
    }
  ...
  return <main className="app-shell">
    {screen === 'start' && <Start board={board} setBoard={setBoard} subject={subject} setSubject={setSubject} tier={tier} setTier={setTier} start={start} available={available.length >= DIAGNOSTIC_LENGTH && Boolean(boundarySet)} />}
    {screen === 'quiz' && current && <Quiz q={current} index={answers.length} selected={selected} choose={setSelected} next={answer} back={restart} board={board} subject={subject} tier={tier} />}
    {screen === 'result' && <Result grade={grade} percentage={Math.round(weightedScore * 100)} projectedMark={projectedMark} confidence={confidence} weakTopics={weakTopics} share={() => setScreen('share')} board={board} subject={subject} tier={tier} restart={restart} />}
...
function Start({ board, setBoard, subject, setSubject, tier, setTier, start, available }) {
  return <section className="screen start-screen">
    ...
    <button className="primary" disabled={!available} onClick={start}>{available ? <>Start Diagnostic <ArrowRight size={18}/></> : 'This combination is not ready yet'}</button>
    <div className="tiny-note">{available ? '~ 2 taps to start' : 'Production content and a verified boundary set are required.'}</div>
  </section>;
}
...
function Quiz({ q, index, selected, choose, next, back, board, subject, tier }) {
  return <section className="screen quiz-screen">
    <header className="quiz-header"><button className="icon-btn" onClick={back} aria-label="Exit diagnostic"><ArrowLeft/></button><span>{subject}&nbsp; • &nbsp;{board}&nbsp; • &nbsp;{tier}</span><button className="text-btn" onClick={back}>Exit</button></header>
...
function Result({grade, percentage, projectedMark, confidence, weakTopics, share, board, subject, tier, restart}) {
  const gradeLabel = grade ?? '—';
  return <section className="screen result-screen"><header><Logo/><button className="icon-btn" onClick={share} aria-label="Share result"><Share2/></button></header><div className="result-title">Your estimated grade</div><div className="grade-wrap"><div className="orbit"/><div className="grade">{gradeLabel}</div></div><div className="solid">{grade ? `SOLID ${grade}` : 'NOT ENOUGH DATA'}</div><p className="disclaimer">Based on your performance in this diagnostic. This is a practice-based estimate, not an official grade. The projected mark is a calibration aid, not an exam mark.</p><div className="stats"><Stat value={`${percentage}%`} label="Diagnostic score"/><Stat value={confidence} label="Confidence in this grade"/><Stat value={projectedMark ?? '—'} label="Projected exam mark"/></div><button className="primary" onClick={share}><Share2 size={18}/> Share Your Result</button><button className="secondary" onClick={() => document.getElementById('breakdown')?.scrollIntoView({behavior:'smooth'})}>View Topic Breakdown</button>{weakTopics.length > 0 && <div id="breakdown" className="weak"><span>Focus next</span>{weakTopics.map(t => <b key={t.specPoint}>{formatSpecPoint(t.specPoint)}</b>)}</div>}<div className="result-context">{board} · {subject} · {tier} · {BOUNDARY_YEAR} {BOUNDARY_SERIES}</div><button className="secondary restart" onClick={restart}>Take it again</button></section>;
}
function formatSpecPoint(value) { return value.replace(/^MATHS-/, '').replace(/-SEED$/, '').replaceAll('-', ' ').toLowerCase(); }
